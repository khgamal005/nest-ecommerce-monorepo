...
const OrderStatusStepper: React.FC<OrderStatusStepperProps> = ({
  ...
  if (isCancelled) {
    return (
      <div className="w-full py-4 md:py-8 px-6 md:px-12">
  ...
  // Find the index of the current status by matching Arabic labels or Enum values
  ...
  if (activeIndex === -1) {
    if (normalizedCurrentStatus === 'pending') activeIndex = 0;
    else if (
      normalizedCurrentStatus === 'processing' ||
      normalizedCurrentStatus === 'packed'
    )
      activeIndex = 1;
    else if (normalizedCurrentStatus === 'shipped') activeIndex = 2;
    else if (
  ...
  // We want the bar to reach exactly the center of the active step's position.
  // With flex justify-between, the items are at 0%, 25%, 50%, 75%, 100% of the container width (for 5 items).
    ...
    <div className="w-full py-4 md:py-8 px-6 md:px-12">
      <div className="relative w-full">
        ...
        <div className="hidden md:block relative py-8">
          ...
          {/* Active Progress Bar */}
          <div
            className="absolute top-1/2 left-0 h-2 bg-linear-to-r from-green-400 to-green-600 rounded-full -translate-y-1/2 transition-all duration-700 ease-out"
                ...
                <div key={step} className="flex flex-col items-center">
                  <div
                    className={`w-5 h-5 rounded-full border-4 z-10 transition-all duration-500 ${
                      isActive
                        ? 'bg-green-500 border-green-100 shadow-[0_0_15px_rgba(34,197,94,0.4)]'
        ...
        <div className="block md:hidden relative pl-8 py-2">
          ...
          <div className="absolute left-[3.5px] top-4 bottom-4 w-1 bg-gray-100 rounded-full" />

          {/* Active Progress Bar (Vertical) */}
          <div
            className="absolute left-[3.5px] top-4 bg-linear-to-b from-green-400 to-green-600 rounded-full transition-all duration-700 ease-out"
            style={{
              height:
                activeIndex === 0
                  ? '0%'
                  : `${(activeIndex / (steps.length - 1)) * 100}%`,
              bottom: activeIndex === steps.length - 1 ? '16px' : 'auto', // Adjust for last item
              width: '4px',
            }}
          />
            ...
            {steps.map((step, index) => {
              ...
              return (
                <div key={step} className="flex items-center gap-4 relative">
                  <div
                    className={`w-3 h-3 rounded-full border-2 z-10 -ml-[29px] transition-colors duration-500 ${
                      isActive
